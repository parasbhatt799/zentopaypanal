                  ...
                  <td className="py-3.5 px-4 font-mono text-xs">
                    {tx.type === 'debit' ? (
                      <div className="flex flex-col gap-1">
                        {tx.clientTxnId && (
                          <div className="flex items-center gap-1.5 group/ord">
                            ...
                            <button
                              type="button"
                              onClick={() => copyToClipboard(tx.clientTxnId!, `adm-st-ord-${tx.id}`)}
                              title="Copy Order ID"
                              className="opacity-60 hover:opacity-100 transition-opacity p-0.5 text-violet-400 hover:text-white"
                            >
                              {copiedId === `adm-st-ord-${tx.id}` ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                            </button>
                          </div>
                        )}
                        {tx.bbpsRef && (
                          <div className="flex items-center gap-1.5 group/bbps">
                            ...
                            <button
                              type="button"
                              onClick={() => copyToClipboard(tx.bbpsRef!, `adm-st-bbps-${tx.id}`)}
                              title="Copy BBPS Ref"
                              className="opacity-60 hover:opacity-100 transition-opacity p-0.5 text-emerald-400 hover:text-white"
                            >
                              {copiedId === `adm-st-bbps-${tx.id}` ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                            </button>
                          </div>
                        )}
                      ...
                      <div className="flex items-center gap-1.5 group/utr">
                        ...
                        <button
                          type="button"
                          onClick={() => copyToClipboard(tx.refId, `adm-st-utr-${tx.id}`)}
                          title="Copy UTR"
                          className="opacity-60 hover:opacity-100 transition-opacity p-0.5 text-slate-400 hover:text-white"
                        >
                          {copiedId === `adm-st-utr-${tx.id}` ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                        </button>
                      </div>
                    )}
                  </td>
